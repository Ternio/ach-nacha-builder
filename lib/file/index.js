...
File.prototype.generateBatches = function (doneGenerateBatches) {
    ...
    // initialize the row count of our batch to include the file header and file control
    var rowCount = 2;

    var entryHash = 0;
    var traceCount = 0;
    var fileControlEntryAddendaCount = 0;
    ...
    async.each(this._batches, function (batch, doneBatchIteration) {
        ...
        async.each(batch._entries, function (entry, doneEntryIteration) {
            ...
            fileControlEntryAddendaCount += entry.getRecordCount();

            // increment our row count which is used to calculate our block count, where block count represents
            // the total number of physical blocks on the file, including the file header and file control records
            rowCount += entry.getRecordCount();
            ...
            // if we have no batch entries, we have no batch
            if (!batch._entries.length || batch._entries.length === 0) {
                doneBatchIteration();
                return;
            }
            ...
            // Bump the number of rowCount only for batches with at least one entry
            // to include the batch header and batch control record
            rowCount = rowCount + 2;
        ...
        self.control.addendaCount.value = fileControlEntryAddendaCount;

        // increment the batch control block count
        // represents the total number of records in the ACH file, rounded up to the nearest 10, then divided by 10
        self.control.blockCount.value = utils.getNextMultiple(rowCount, 10) / 10;
        ...
        doneGenerateBatches(result, rowCount);
    });
};
    ...
    self.generateHeader(function (headerString) {
        self.generateBatches(function (batchString, rows) {
            self.generateControl(function (controlString) {
                ...
                // ACH files must be generated as complete blocks of 10 records each, so this represents
                // padding our file to to ensure we generate complete blocks
