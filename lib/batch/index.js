...
function Batch(options) {
	...
	if (options.companyDescriptiveDate) {
		if (options.companyDescriptiveDate instanceof Date) {
			this.header.companyDescriptiveDate.value = utils.formatDescriptiveDate(options.companyDescriptiveDate);
		} else {
...
Batch.prototype.addEntry = function(entry) {
	...
	// (21, 22, 23, 24, 26, 27, 28, 29, 32, 33, 34, 37, 38 & 39)
	var creditCodes = ['21', '22', '23', '24', '32', '33', '34'];
	var debitCodes = ['26', '27', '28', '29', '37', '38', '39'];
	...
	async.each(this._entries, function(entry, done) {
		...
		if(_.includes(creditCodes, entry.fields.transactionCode.value)) {
			...
			console.error('Transaction code did not match or is not supported yet.');
		}
	}, function(err) {
